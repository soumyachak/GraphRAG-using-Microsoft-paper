...
You are a helpful assistant responsible for generating a comprehensive summary of the data provided below.
Given two entities, and a list of relationships between them, all related to the same two entities.
Please concatenate all of these into a single, comprehensive relationship. Make sure to include information collected from all the relationships.
If the provided relationships are contradictory, please resolve the contradictions and provide a single, coherent summary.

#######
-Data-
Entities: {entity_name1, entity_name2}
Description List: {relationships_list}
#######
Output:
'''

GRAPH_OF_THOUGHTS_PROMPT = '''
-Goal-
Given a text document that is potentially relevant to this activity and a set of questions, create a knowledge graph which will help in answering the questions from the text document containing all entities and all relationships among the identified entities.

-Steps-
1. Study the "Context" carefully and generate answer by thinking step by step for each question in the "Question_Set". Identify all entities from the generated answer. For each identified entity, extract the following information:
- entity_name: Name of the entity, capitalized
...
Question_Set:
1. What is the effect of Taylor on Alex?
2. What is common between Jordan and Alex? How is Cruz related to the same?
################
Output:
("entity"{tuple_delimiter}"Alex"{tuple_delimiter}"person"{tuple_delimiter}"Alex is a character who experiences frustration and is observant of the dynamics among other characters."){record_delimiter}
("entity"{tuple_delimiter}"Taylor"{tuple_delimiter}"person"{tuple_delimiter}"Taylor is portrayed with authoritarian certainty and shows a moment of reverence towards a device, indicating a change in perspective."){record_delimiter}
...
Question_Set:
1. Who drives the decision making of the team?
2. Is the team directly involved in operation?
#############
Output:
("entity"{tuple_delimiter}"Washington"{tuple_delimiter}"location"{tuple_delimiter}"Washington is a location where communications are being received, indicating its importance in the decision-making process."){record_delimiter}
("entity"{tuple_delimiter}"Operation: Dulce"{tuple_delimiter}"mission"{tuple_delimiter}"Operation: Dulce is described as a mission that has evolved to interact and prepare, indicating a significant shift in objectives and activities."){record_delimiter}
...
GRAPH_OF_THOUGHTS_PREDICTION_PROMPT = '''
You are an helpful Question Answering AI assistant based on the relevant context provided.
In the context, you are provided information about nodes and relationships from a relevant portion of knowledge graph according to the question asked. The information is provided in the below format:
1. Format each entity as ("entity"{tuple_delimiter}<entity_name>{tuple_delimiter}<entity_type>{tuple_delimiter}<entity_description>)
2. Format each relationship as ("relationship"{tuple_delimiter}<source_entity>{tuple_delimiter}<target_entity>{tuple_delimiter}<relationship_description>{tuple_delimiter}<relationship_strength>)

#########################################################
Answer the question based on the below context provided and predict the most relevant answer.
